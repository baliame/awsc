...
class ImportContextDialog(SessionAwareDialog):
  def __init__(self, parent, alignment, dimensions, caller=None, *args, **kwargs):
    ...
    if creds is None or not creds.access_key or not creds.secret_key:
      Common.Session.set_message('No valid credentials in environment.', Common.color('message_error'))
      self.close()
      return
    self.access_key = creds.access_key
    self.secret_key = creds.secret_key
    self.access_key_field = DialogFieldLabel([('Access key: ', Common.color('generic')), (creds.access_key, Common.color('highlight'))], centered=False)
    self.add_field(self.access_key_field)
    self.secret_key_field = DialogFieldLabel([('Secret key: ', Common.color('generic')), ('*' * len(creds.secret_key), Common.color('highlight'))], centered=False)
  ...
  def accept_and_close(self):
    if self.name_field.text == '':
      ...
      return
    try:
      sts = Common.Session.service_provider.whoami(keys={'access': self.access_key, 'secret': self.secret_key})
    ...
    Common.Configuration.add_or_edit_context(self.name_field.text, sts['Account'], self.access_key, self.secret_key)
    self.close()
...
class AddContextDialog(SessionAwareDialog):
  ...
  def accept_and_close(self):
    ...
    if self.secret_key_field.text == '':
      ...
      return
    try:
      sts = Common.Session.service_provider.whoami(keys={'access': self.access_key_field.text, 'secret': self.secret_key_field.text})
    except exceptions.ClientError as e:
      self.error_label.text = 'Key verification failed'
      return
    ...
    Common.Configuration.add_or_edit_context(self.name_field.text, sts['Account'], self.access_key_field.text, self.secret_key_field.text)
    self.close()
...
class ContextList(ListControl):
  ...
  def import_context(self, _):
    ImportContextDialog(self.parent, CenterAnchor(0, 0), Dimension('80%|40', '20'), caller=self, weight=-500)
